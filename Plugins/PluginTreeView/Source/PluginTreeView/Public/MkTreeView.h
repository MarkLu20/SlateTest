...
public:
	...
	void Construct(const typename STableRow<ItemType>::FArguments& InArgs, const TSharedRef<STableViewBase>& InOwnerTableView, TArray<FSlateBrush>& expander,  FMargin& expanderPadding)
	{
	...
	void ConstructChildren(ETableViewMode::Type InOwnerTableMode, const TAttribute<FMargin>& InPadding, const TSharedRef<SWidget>& InContent, TArray<FSlateBrush>& expander, FMargin& expanderPadding)
	{
		...
		else
		{
			...
			this->ChildSlot
				...
				.VAlign(VAlign_Fill)
				[
					SNew(SMkExpanderArrow, SharedThis(this), expander, expanderPadding)
